...
/// Trace source id for the main FIRQ line.
const IRQ_TRACE_FIRQ: u8 = 1;
...
impl MainInterrupts {
    ...
    pub(crate) fn write_mask<T: Tracing>(&mut self, value: u8, tracer: &mut T) {
        self.mask = value;
        self.sync_irq_line(tracer);
    }

    /// Sets or clears the timer IRQ pending latch.
    pub(crate) fn set_timer_pending<T: Tracing>(&mut self, pending: bool, tracer: &mut T) {
    ...
    pub(crate) fn set_fdc_pending<T: Tracing>(&mut self, pending: bool, tracer: &mut T) {
    ...
    pub(crate) fn set_keyboard_pending<T: Tracing>(&mut self, pending: bool, tracer: &mut T) {
    ...
    pub(crate) fn set_opn_pending<T: Tracing>(&mut self, pending: bool, tracer: &mut T) {
    ...
    pub(crate) fn set_break<T: Tracing>(&mut self, pressed: bool, tracer: &mut T) {
        self.break_pending = pressed;
        self.sync_firq_line(tracer);
    }

    /// Clears the timer IRQ pending latch.
    pub(crate) fn ack_timer<T: Tracing>(&mut self, tracer: &mut T) {
        self.timer_pending = false;
        self.sync_irq_line(tracer);
    }

    /// Reads the `0xFD03` IRQ status register and clears timer/printer status.
    pub(crate) fn read_status<T: Tracing>(&mut self, tracer: &mut T) -> u8 {
        let mut value = 0xFF;
    ...
    pub(crate) fn raise_sub_attention<T: Tracing>(&mut self, tracer: &mut T) {
        self.sub_attention = true;
        self.sync_firq_line(tracer);
    }

    /// Reads the `0xFD04` FIRQ status register and clears sub attention.
    pub(crate) fn read_firq_status<T: Tracing>(&mut self, tracer: &mut T) -> u8 {
        let mut value = 0xFF;
    ...
    fn sync_irq_line<T: Tracing>(&mut self, tracer: &mut T) {
        let new_line = self.irq_line();
        if new_line != self.irq_line {
            self.irq_line = new_line;
            if new_line {
                tracer.trace_irq_raise(IRQ_TRACE_MAIN);
            } else {
                tracer.trace_irq_clear(IRQ_TRACE_MAIN);
            }
        }
    }

    /// Updates the traced FIRQ line state.
    fn sync_firq_line<T: Tracing>(&mut self, tracer: &mut T) {
        let new_line = self.firq_line();
        if new_line != self.firq_line {
            self.firq_line = new_line;
            if new_line {
                tracer.trace_irq_raise(IRQ_TRACE_FIRQ);
            } else {
                tracer.trace_irq_clear(IRQ_TRACE_FIRQ);
            }
        }
    }
}
