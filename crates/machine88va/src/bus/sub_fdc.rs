...
//! data rate via `EventVA::FdcDrqByte`, and finalizes commands.
...
impl Pc88VaBus {
    ...
    pub(crate) fn assert_fdc_terminal_count(&mut self) {
        ...
        self.scheduler
            .schedule(EventVA::FdcTcClear, self.current_cycle + pulse);
        self.update_next_event_cycle();
    }
...
mod tests {
    ...
    use super::*;
    use crate::{bus::test_support::test_bus, scheduler::EventVA};
    ...
    /// main IRQ asserts.
    fn fire_fdc_completion(bus: &mut Pc88VaBus, event: EventVA) {
    ...
    fn dma_read_raises_main_irq_11() {
        ...
        fire_fdc_completion(&mut bus, EventVA::FdcResultComplete);
    ...
    fn recalibrate_raises_main_irq_in_dma_mode() {
        ...
        fire_fdc_completion(&mut bus, EventVA::FdcSeekComplete);
        assert!(bus.pic.has_pending_irq());
        assert_eq!(bus.pic.acknowledge(), FDC_IRQ_VECTOR);
