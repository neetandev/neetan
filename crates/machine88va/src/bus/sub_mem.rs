//! Floppy sub-CPU (PC80S31K) memory: a flat 64 KiB space.
//!
//! `vasubsys.rom` at 0x0000-0x1FFF, a fixed init pattern at 0x2000-0x7FFF
//! (only 0x4000-0x7FFF is writable RAM), and 0xFF everywhere above 0x8000.
//! The init pattern is the power-on contents the sub-CPU ROM expects to find;
//! reproducing it keeps the sub-CPU boot deterministic.

/// Floppy sub-CPU 64 KiB memory.
...
impl SubMemory {
    /// Creates the sub memory in its power-on state (no sub-CPU ROM yet).
    pub(crate) fn new() -> Self {
        ...
        // JR * at the reset vector idles the sub CPU until the ROM is loaded.
        mem[0x0000] = 0x18;
        mem[0x0001] = 0xFE;
    ...
    /// Loads the floppy sub-CPU ROM (8 KiB) at 0x0000-0x1FFF.
