...
impl Pc88VaBus {
    ...
    pub(crate) fn schedule_pit_timer0(&mut self) {
        ...
        self.scheduler
            .schedule(EventVA::PitTimer0, self.current_cycle + period);
    }
    ...
    pub(crate) fn write_timer3_ctrl(&mut self, value: u8) {
        ...
        if value & 0x80 != 0 {
            ...
            self.scheduler.cancel(EventVA::Timer3);
    ...
    fn schedule_timer3(&mut self) {
        ...
        self.scheduler
            .schedule(EventVA::Timer3, self.current_cycle + period);
    }
    ...
    fn process_events(&mut self) {
        ...
        for event in &due {
            match event.kind {
                ...
                EventVA::TspFrame => self.on_tsp_frame(),
                EventVA::Sysp4Vsync => self.on_sysp4_vsync(),
                EventVA::SgpComplete => self.on_sgp_complete(),
                EventVA::FdcDrqByte => self.on_fdc_drq_byte(event.fire_cycle),
    ...
    fn sgp_reset(&mut self) {
        self.sgp.reset();
        self.scheduler.cancel(EventVA::SgpComplete);
        self.update_next_event_cycle();
    }
    ...
    fn on_tsp_frame(&mut self) {
        match self.tsp.frame_phase {
            FramePhase::DisplayStart => {
                ...
                self.scheduler
                    .schedule(EventVA::TspFrame, self.current_cycle + self.tsp.dispclock);
            ...
            FramePhase::Vsync => {
                ...
                self.scheduler
                    .schedule(EventVA::TspFrame, self.current_cycle + self.tsp.vsyncclock);
            }
        }
    }
    ...
    fn on_sysp4_vsync(&mut self) {
        match self.tsp.sysp4_phase {
            ...
            Sysp4Phase::Start => {
                ...
                self.scheduler
                    .schedule(EventVA::Sysp4Vsync, self.current_cycle + 6);
            }
            Sysp4Phase::Int => {
                self.pic.set_irq(2);
            }
