...
impl std::fmt::Display for MemoryWaitSwitch {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl std::str::FromStr for MemoryWaitSwitch {
    type Err = String;
...
/// 8 MHz wait-state mode. `Fast` is the high-speed mode that omits the extra
/// 8 MHz wait state; `Compatible` keeps it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl std::fmt::Display for EightMhzWaitMode {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl std::str::FromStr for EightMhzWaitMode {
    type Err = String;
