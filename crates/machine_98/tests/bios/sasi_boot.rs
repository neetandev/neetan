use super::{
    create_machine_f_hdd, create_machine_ra_hdd, create_machine_vm_empty_hdd,
    create_machine_vm_hdd, create_machine_vx_hdd, read_ram_u16,
};
...
fn sasi_initialization_f() {
    ...
    assert_eq!(
        disk_equip, 0x0100,
        "DISK_EQUIP should have SASI drive 0 without FDD bits (got {disk_equip:#06X})"
    );
    assert_eq!(
        state.memory.ram[BDA_BOOT_DEVICE], 0x80,
        "BOOT_DEVICE should be 0x80 for SASI-0"
    );
    assert_eq!(state.memory.ram[BDA_F2HD_MODE], 0x00, "F2HD_MODE");
