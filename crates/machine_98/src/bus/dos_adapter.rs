...
//! `Pc9801Memory`) to implement the shared HLE DOS bridge traits.
...
pub(super) struct DosCpuAccess<'a, C: Cpu>(pub &'a mut C);

impl<C: Cpu> CpuAccess for DosCpuAccess<'_, C> {
    fn ax(&self) -> u16 {
        self.0.ax()
    }
...
pub(super) struct DosMemoryAccess<'a> {
...
pub(super) struct DosCursorAccess<'a>(pub &'a mut GdcState);

impl CursorAccess for DosCursorAccess<'_> {
