...
const FONT_ROM_SIZE: usize = software_renderer::PC98_CGROM_SIZE;
...
impl Pc9801Memory {
    ...
    pub(crate) fn load_font_rom(&mut self, data: &[u8]) {
        if !software_renderer::expand_v98_font_rom(&mut self.font_rom, data) {
            common::warn!(
                "Font ROM too small ({} bytes, expected {})",
                data.len(),
                software_renderer::V98_FONT_ROM_SIZE
            );
        }
    }
...
mod tests {
    use common::MachineModel;
    use software_renderer::V98_FONT_ROM_SIZE;

    use super::Pc9801Memory;
