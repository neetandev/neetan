...
//! memory access goes through the [`SgpMemory`] interface.

const SGP_INTF: u8 = 0x04;
const SGP_ABORT: u8 = 0x02;
const SGP_BUSY: u8 = 0x01;

/// Memory interface used by the SGP command processor.
pub trait SgpMemory {
    /// Reads one native-order word from the SGP address space.
    fn read_word(&self, address: u32) -> u16;

    /// Writes one native-order word to the SGP address space.
    fn write_word(&mut self, address: u32, value: u16);
}

/// External effects caused by an SGP control-register write.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct SgpControlEffect {
    /// Whether a pending completion event must be cancelled.
    pub abort: bool,
    /// Whether the machine must assert the SGP interrupt line.
    pub raise_interrupt: bool,
    /// Whether the machine must clear the SGP interrupt line.
...
impl SgpBlock {
    ...
    fn read_word(&mut self, mem: &impl SgpMemory) {
        self.buf = mem.read_word(self.nextaddress).swap_bytes();
...
impl SgpState {
    ...
    /// Resets the SGP command processor.
    ...
    pub fn write_address_byte(&mut self, port: u16, value: u8) {
    ...
    pub fn write_trigger(&mut self, value: u8) -> bool {
    ...
    /// Reads the SGP control register.
    pub fn control(&self) -> u8 {
        self.ctrl
    }

    /// Reads the SGP busy register.
    pub fn busy(&self) -> u8 {
        self.busy
    }

    /// Applies an SGP control-register write and returns its machine effects.
    pub fn write_control(&mut self, value: u8) -> SgpControlEffect {
        self.ctrl = value & (SGP_INTF | SGP_ABORT);
        let abort = self.ctrl & SGP_ABORT != 0;
        if abort {
            self.busy &= !SGP_BUSY;
        }
        let raise_interrupt = abort && self.ctrl & SGP_INTF != 0 && !self.intreq;
        if raise_interrupt {
            self.intreq = true;
        }
        let clear_interrupt = self.ctrl & SGP_INTF == 0 && self.intreq;
        if clear_interrupt {
            self.intreq = false;
        }
        SgpControlEffect {
    ...
    /// Completes a run and returns whether the interrupt line must be raised.
    pub fn complete(&mut self) -> bool {
        self.busy &= !SGP_BUSY;
        if self.ctrl & SGP_INTF != 0 && !self.intreq {
            self.intreq = true;
            true
        } else {
            false
    ...
    pub fn execute(&mut self, mem: &mut impl SgpMemory) -> u64 {
        self.cycles = 0;
    ...
    fn fetch_command(&mut self, mem: &mut impl SgpMemory) -> bool {
        let cmd = mem.read_word(self.pc);
    ...
    fn cmd_set_work(&mut self, mem: &impl SgpMemory) {
        self.workmem = u32::from(mem.read_word(self.pc) & 0xFFFE)
            | (u32::from(mem.read_word(self.pc.wrapping_add(2))) << 16);
        self.pc = self.pc.wrapping_add(4);
        self.spend(23 * 2);
    }

    fn cmd_set_source(&mut self, mem: &impl SgpMemory) {
        fetch_block(mem, self.pc, &mut self.src);
        self.pc = self.pc.wrapping_add(12);
        self.spend(106 * 2);
    }

    fn cmd_set_destination(&mut self, mem: &impl SgpMemory) {
        fetch_block(mem, self.pc, &mut self.dest);
        self.pc = self.pc.wrapping_add(12);
        self.spend(106 * 2);
    }

    fn cmd_set_color(&mut self, mem: &impl SgpMemory) {
        self.color = mem.read_word(self.pc);
        self.pc = self.pc.wrapping_add(2);
        self.spend(10 * 2);
    }

    fn cmd_bitblt(&mut self, mem: &impl SgpMemory) {
        self.bltmode = mem.read_word(self.pc);
        self.pc = self.pc.wrapping_add(2);
        self.spend(338 * 2);
    ...
    fn cmd_patblt(&mut self, mem: &impl SgpMemory) {
        self.bltmode = mem.read_word(self.pc);
        self.pc = self.pc.wrapping_add(2);
        self.spend(338 * 2);
    ...
    fn cmd_line(&mut self, mem: &impl SgpMemory) {
        self.bltmode = mem.read_word(self.pc);
        self.pc = self.pc.wrapping_add(2);
        fetch_block(mem, self.pc, &mut self.dest);
        self.pc = self.pc.wrapping_add(12);
    ...
    fn cmd_cls(&mut self, mem: &impl SgpMemory) {
        self.clsaddr = (u32::from(mem.read_word(self.pc))
            | (u32::from(mem.read_word(self.pc.wrapping_add(2))) << 16))
            & 0xFFFF_FFFE;
        self.clscount = u32::from(mem.read_word(self.pc.wrapping_add(4)))
            | (u32::from(mem.read_word(self.pc.wrapping_add(6))) << 16);
    ...
    fn cmd_scan(&mut self, mem: &impl SgpMemory, left: bool) {
        self.spend(10 * 2);
        ...
        for x in 0..self.dest.width {
            ...
            let raw = mem.read_word(address).swap_bytes();
    ...
    fn init_src_line(&mut self, mem: &impl SgpMemory) {
    ...
    fn init_src_line_hd(&mut self, mem: &impl SgpMemory) {
        let dot = if self.bltmode & BLTMODE_SF != 0 {
            if self.dest.dot > self.src.dot {
    ...
    fn init_dest_line_hd(&mut self, mem: &impl SgpMemory) {
        self.newval = 0;
        self.newvalmask = 0;
    ...
    fn write_dest2(&self, mem: &mut impl SgpMemory) {
        // The undocumented transparent mode 3 never transfers.
        if self.bltmode & BLTMODE_TP == 0x0300 {
            return;
        }
        let dest = mem.read_word(self.dest.nextaddress).swap_bytes();
        ...
        mem.write_word(self.dest.nextaddress, dat.swap_bytes());
    }

    fn exec_bitblt(&mut self, mem: &mut impl SgpMemory) {
    ...
    fn exec_bitblt_hd(&mut self, mem: &mut impl SgpMemory) {
        let bpp = BPP[self.dest.scrnmode];
        let bpp_u = bpp as u32;
        let pixmask = (!(0xFFFFu32 << bpp_u)) as u16;
        let extpix = self.src.scrnmode == 0 && self.dest.scrnmode != 0;
    ...
    fn advance_blit_line(&mut self, mem: &impl SgpMemory, hd: bool) {
    ...
    fn exec_cls(&mut self, mem: &mut impl SgpMemory) {
        mem.write_word(self.clsaddr, self.color);
    ...
    fn exec_line_x(&mut self, mem: &mut impl SgpMemory) {
    ...
    fn exec_line_y(&mut self, mem: &mut impl SgpMemory) {
...
fn fetch_block(mem: &impl SgpMemory, address: u32, block: &mut SgpBlock) {
    let dat = mem.read_word(address);
    block.scrnmode = (dat & 0x03) as usize;
    block.dot = (i32::from(dat >> 4)) & (DOTCOUNTMAX[block.scrnmode] - 1);
    block.width = mem.read_word(address.wrapping_add(2)) & 0x3FFF;
    block.height = mem.read_word(address.wrapping_add(4));
    block.fbw = (mem.read_word(address.wrapping_add(6)) & 0xFFFE) as i16;
    block.address = u32::from(mem.read_word(address.wrapping_add(8)) & 0xFFFE)
        | (u32::from(mem.read_word(address.wrapping_add(10))) << 16);
    if block.fbw < 0 {
