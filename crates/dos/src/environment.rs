...
pub(crate) fn entries(state: &DosState, memory: &dyn MemoryAccess) -> Vec<Vec<u8>> {
...
fn read_block(state: &DosState, memory: &dyn MemoryAccess) -> Option<EnvironmentBlock> {
...
fn env_segment(state: &DosState, memory: &dyn MemoryAccess) -> u16 {
