...
impl<T: TraceSink> Pc6000Bus<T> {
    ...
    pub fn io_read(&mut self, port: u16) -> (u8, bool) {
        match port & 0xF0 {
            ...
            0xA0 => (self.psg_read(port), true),
            0xB0 if !self.model.is_sr() && self.model.has_builtin_fdd() && port & 0xFF == 0xB2 => {
                (self.fdc_read(port), true)
            }
            0xD0 if self.model.has_builtin_fdd() => (self.fdc_read(port), true),
    ...
    /// open bus.
    fn io_read_extended(&mut self, port: u16) -> (u8, bool) {
        match self.model {
            Pc6000Model::Pc6001 => (OPEN_BUS, false),
    ...
    fn io_read_mk2(&mut self, port: u16) -> (u8, bool) {
    ...
    fn io_read_sr(&mut self, port: u16) -> (u8, bool) {
        let value = match port & 0xFF {
            0x60..=0x67 => self
                .memory
                .sr()
