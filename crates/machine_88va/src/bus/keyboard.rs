...
impl KeyboardVa {
    ...
    pub(crate) fn set_key(&mut self, row: usize, column: usize, pressed: bool) {
    ...
    pub(crate) fn read_row(&self, row: usize) -> u8 {
...
impl<T: common::TraceSink> Pc88VaBus<T> {
    ...
    /// is also translated to its 88-compatible matrix cell so the always-on scan
    /// matrix (ports 0x00-0x0E) read by the boot ROM, the setup menu, and V1/V2
    /// BASIC stays consistent.
    ...
    /// Reads the keyboard data port (0x1C1). The keyboard IRQ is edge-triggered, so
    /// each queued code needs its own edge: re-assert the line when codes remain and
    /// clear it once the queue drains.
